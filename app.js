...
function verifyExcelFile(workbook) {
  ...
  if (!workbook.Sheets[expectedSheetName]) {
    throw new Error('O arquivo Excel não corresponde ao modelo esperado: nome da planilha incorreto.');
  }
}
...
async function excelToJson(excelFilePath, jsonFilePath) {
  try {
    ...
    // Converter a planilha para JSON começando na linha 3
    const sheetName = 'Planilha1';
    const sheet = workbook.Sheets[sheetName];
    let jsonData = XLSX.utils.sheet_to_json(sheet, { defval: null, range: 2 });

    // Remover propriedades com valores null e converter opções
    jsonData = jsonData.map(data => convertOptions(removeNullProperties(data)));
